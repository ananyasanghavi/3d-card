    ...
    whileTap={{cursor:'grabbing'}}
     ...
     border-white px-[40px] py-[24px] cursor-grab relative'>
      ...
      <p className='max-w-[300px] text-[#000000] mb-6'>
        Taking a classic look of heritage Nike running into new realm, the Nike Air Max Pre-Day brings
        you a fast-paced look that's ready for today's world.
      </p>
      </div>

      <div className='flex items-center gap-x-[20px] mb-12'>
        <button className=' bg-[#2d2b2c] text-white text-base font-medium py-[16px]
         px-[40px] rounded-lg'>
        Add to Bag
        </button>
      ...
      <ul className='flex gap-x-[10px]'>
        ...
        {colors.map((color,index) => {
          return (<li key={index} style={{backgroundColor:color.value}} 
      ...
      <motion.div style={{x,y,rotateX,rotateY,z:100000}}
       className='absolute top-12 -right-64 w-[620px]'>
        <img src={NikeImg} draggable='false'/>
      </motion.div>
    </motion.div>
  </div>;
};
